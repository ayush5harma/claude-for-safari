...
| `claude_safari_navigate` | Sends a tab to a URL, or opens a new one. **A new tab opens in the background** (`active: false`) of the owner's current window unless asked otherwise, so the tab a window is showing never changes. Sending an existing tab (`tabId`, no `newTab`) first installs a capture listener that stops the page's `beforeunload`, because Safari brings a background tab to the front to show its "Leave page?" sheet. That disarms a page's unsaved-changes guard on every existing-tab navigate, so navigate only tabs you opened, never the owner's; a capture-phase listener the page registered earlier can still raise the sheet. | `url`, `newTab`, `active`, `windowId`, `tabId`, `profile` |
...
| `claude_safari_upload` | Files onto an `<input type=file>` (a `DataTransfer`), then `input`/`change`. Each file is either `{path}`, an absolute path on the Mac running the MCP server, which reads it and sends base64 (name defaults to the basename, type to one from the extension), so a hosted hub never sees a path. The file's real path (symlinks resolved) must be under an upload root: `~/Downloads`, `~/Desktop`, `~/Documents`, `~/Github`, `~/.cache/system-config` and the temp dir, or `CLAUDE_SAFARI_UPLOAD_ROOTS` (colon-separated) when set; `~/.ssh`, `~/.gnupg`, `~/.aws`, `~/.mcp-auth`, `~/.netrc` and anything named like an age key are refused even inside a root. This server reads the file itself, so Claude Code's own Read permissions do not apply to it; or `{name, base64}` inline, realistic only for small files since it passes through the model's context. A chat panel turn cannot use `path`: its prompt carries page text, and a page must not be able to ask for a local file. At most 10 files and 24 MB decoded per call (a 20 MB APK is the largest measured through Safari); a single-file input takes one. | `selector`, `files: [{path, name?, type?} or {name, type, base64}]`, `tabId`, `profile`, `frameId`, `frameUrl` |
