...
| `claude_safari_upload` | Files onto an `<input type=file>` (a `DataTransfer`), then `input`/`change`. Each file is either `{path}`, an absolute path on the Mac running the MCP server, which reads it and sends base64 (name defaults to the basename, type to one from the extension), so a hosted hub never sees a path; or `{name, base64}` inline, realistic only for small files since it passes through the model's context. At most 10 files and 48 MB decoded per call; a single-file input takes one. | `selector`, `files: [{path, name?, type?} or {name, type, base64}]`, `tabId`, `profile`, `frameId`, `frameUrl` |
